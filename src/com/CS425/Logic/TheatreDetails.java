...
public class TheatreDetails {
	...
	private static boolean buyTicketGuest(String theatre) {
		...
		if(DBTheatreDetails.purchaseTicketViaCC(gDetails, selectedMovie, quantity)){
			...
			return Invoice.printInvoice(selectedMovie, 0, gDetails.getCardNumber(), theatre, quantity, gDetails.getEmail());
		}
		else
		...
		while(true){
			if(option.equals("1")){
				if(!processPurchase(true, selectedMovie, quantity, userD, userCC)){
					...
					return Invoice.printInvoice(selectedMovie, userD.getMemberId(), userCC.getCardNumber(), theatre, quantity, null);
				}
			}// if(option == "1")
			if(option.equals("2"))
